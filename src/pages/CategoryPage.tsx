...
export const CategoryGrid: React.FC<CategoryGridProps> = ({ onOpenCreate, onOpenView }) => {
  ...
  const handleCreateClick = (e: React.MouseEvent, cat: CategoryItem) => {
    e.preventDefault();
    e.stopPropagation();
    if (typeof onOpenCreate === 'function') {
      onOpenCreate(cat.title, cat.title);
    }
  };

  const handleViewClick = (e: React.MouseEvent, cat: CategoryItem) => {
    e.preventDefault();
    e.stopPropagation();
    if (typeof onOpenView === 'function') {
      onOpenView(cat.title, cat.title);
    }
  };
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 xl:gap-5">
        {CATEGORIES.map((cat: CategoryItem) => (
          ...
          >
            <div>
              <div className="flex items-center justify-between mb-4">
                <div className="p-2.5 rounded-lg bg-stone-950 border border-stone-800 group-hover:border-amber-500/30 transition-colors">
                  {iconMap[cat.icon] || <Sparkles className="w-5 h-5 text-amber-500" />}
                </div>
            ...
            {/* Action Buttons */}
            <div className="grid grid-cols-2 gap-1.5 xl:gap-2 pt-3 border-t border-stone-800/60 relative z-20">
              <button
                type="button"
                onClick={(e) => handleCreateClick(e, cat)}
                className="flex items-center justify-center gap-1 sm:gap-1.5 px-1.5 sm:px-2 py-2 rounded-lg bg-amber-500 hover:bg-amber-400 text-stone-950 text-[10px] sm:text-xs font-bold transition-all active:scale-[0.98] whitespace-nowrap cursor-pointer"
              >
              ...
              <button
                type="button"
                onClick={(e) => handleViewClick(e, cat)}
                className="flex items-center justify-center gap-1 sm:gap-1.5 px-1.5 sm:px-2 py-2 rounded-lg bg-stone-950 hover:bg-stone-800 border border-stone-800 text-stone-300 hover:text-white text-[10px] sm:text-xs font-semibold transition-all active:scale-[0.98] whitespace-nowrap cursor-pointer"
              >
